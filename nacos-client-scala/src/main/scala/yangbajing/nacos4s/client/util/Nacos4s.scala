...
object Nacos4s {
  ...
  def namingService(config: Config): Nacos4sNamingService = namingService(ConfigUtils.toProperties(config))

  def namingService(props: Properties): Nacos4sNamingService = new Nacos4sNamingService(new NacosNamingService(props))

  def namingService(serverList: String): Nacos4sNamingService =
    new Nacos4sNamingService(new NacosNamingService(serverList))
}
